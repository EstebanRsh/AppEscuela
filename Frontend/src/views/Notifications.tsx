...
function Notifications() {
  ...
  useEffect(() => {
    const fetchMessages = async () => {
      if (!token) {
        setError("No se pudo verificar la sesión. Por favor, inicie sesión.");
        setIsLoading(false);
        return;
      }
      ...
      try {
        ...
        const sortedData = data.sort((a: Message, b: Message) => Number(a.is_read) - Number(b.is_read));
  ...
  const markAsRead = async (messageId: number) => {
    try {
      const response = await fetch(`http://localhost:8000/messages/${messageId}/read`, {
        method: 'PUT',
        headers: { 'Authorization': `Bearer ${token}` },
      });
          ...
          <div className="card-body">
            ...
            {!isLoading && !error && (
              messages.length > 0 ? (
                  ...
                  {messages.map((msg) => (
                    <li
                      ...
                      className={`list-group-item d-flex justify-content-between align-items-center ${
                        msg.is_read ? 'list-group-item-dark' : 'list-group-item-warning text-dark'
                      }`}
                    >
                      <div className="me-3">
                      ...
                      {!msg.is_read && (
                        <button className="btn btn-sm btn-outline-success flex-shrink-0" onClick={() => markAsRead(msg.id)}>
                          Marcar como leído
                        </button>
                      )}
                    </li>
                ...
                <div className="alert alert-info">No tienes notificaciones nuevas.</div>
              )
            )}
             <div className="mt-4">
